...
It is not necessary to define `API_TOKEN_PEPPERS` here - this role will automatically create one for you at `{{ netbox_shared_path }}/generated_api_token_peppers`.
The `API_TOKEN_PEPPERS` will then be read from this file on subsequent runs, unless you later do set this in your playbook. Also do set this if you want more than one pepper.
...
`netbox_setuptools_version` can be set to a version constraint to control the version of setuptools that gets installed, e.g. `==37.0.0` or `<=42`.
